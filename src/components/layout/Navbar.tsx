...
export function Navbar({ site }: NavbarProps) {
  ...
  return (
    <>
      <header
        className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 px-6 md:px-10 ${
          isScrolled
            ? "bg-warm-bg/95 py-4 shadow-sm backdrop-blur-xl"
            : "bg-transparent py-6"
        }`}
      >
            ...
            {navLinks.map((link) => (
              <a
                ...
                className={`text-xs font-semibold uppercase tracking-[0.24em] ${
                  isScrolled
                    ? "text-warm-text hover:text-primary"
